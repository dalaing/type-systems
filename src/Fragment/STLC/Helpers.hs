...
tyArr :: AsTySTLC ki ty => Type ki ty a -> Type ki ty a -> Type ki ty a
tyArr = curry $ review _TyArr

tmLam :: (Eq a, AsTmSTLC ki ty pt tm) => a -> Type ki ty a -> Term ki ty pt tm a -> Term ki ty pt tm a
tmLam v ty tm = review _TmLam (ty, abstract1 (review _ATmVar v) . review _Unwrapped $ tm)

tmApp :: AsTmSTLC ki ty pt tm => Term ki ty pt tm a -> Term ki ty pt tm a -> Term ki ty pt tm a
