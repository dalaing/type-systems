...
module Fragment.STLC.Rules.Infer.Unification.Offline (
    STLCInferContext
  , stlcInferRules
  ) where
...
inferTmLam :: (Ord a, AstBound ki ty pt tm, MonadState s m, HasTmVarSupply s, ToTmVar a, MonadReader r m, AsTySTLC ki ty, AsTmSTLC ki ty pt tm, HasTermContext r ki ty a)
...
inferTmApp :: (Eq a, EqRec (ty ki), MonadError e m, AsTySTLC ki ty, AsTmSTLC ki ty pt tm, AsExpectedTyArr e ki ty a, AsExpectedEq e ki ty a)
