...
module Fragment.SystemF.Rules.Infer.Unification.Offline (
    SystemFInferContext
  , systemFInferRules
  ) where
...
inferTmApp :: (Eq a, EqRec (ty ki), MonadError e m, AsTySystemF ki ty, AsTmSystemF ki ty pt tm, AsExpectedTyArr e ki ty a, AsExpectedEq e ki ty a)
...
inferTmLamTy :: (Eq a, AstBound ki ty pt tm, MonadState s m, HasTyVarSupply s, ToTyVar a, AsTySystemF ki ty, AsTmSystemF ki ty pt tm) => (Term ki ty pt tm a -> m (Type ki ty a)) -> Term ki ty pt tm a -> Maybe (m (Type ki ty a))
